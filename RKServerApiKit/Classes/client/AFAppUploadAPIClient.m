...
@implementation AFAppUploadAPIClient

+ (instancetype)sharedClient {
    static AFAppUploadAPIClient *_sharedClient = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSURLSessionConfiguration *configuration = [NSURLSessionConfiguration defaultSessionConfiguration];
        configuration.timeoutIntervalForRequest = 5;
        configuration.timeoutIntervalForResource = 5;
        _sharedClient = [[AFAppUploadAPIClient alloc] initWithSessionConfiguration:configuration];
    });
...
- (NSURLSessionDataTask*)GET:(NSString *)URLString
    ...
    __weak AFAppUploadAPIClient *weakClient = self;
    
    //完整URL
    NSString* URL = [AFAppUploadAPIBaseURLString stringByAppendingString:URLString];
...
//    [request addValue:baseString forHTTPHeaderField:TOKEN];
...
- (NSURLSessionDataTask*)POST:(NSString *)URLString
    ...
    if (token) {
        ...
        NSURLSessionDataTask* dataTask =  [[AFAppUploadAPIClient sharedClient] dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
            BaseResponse *mBaseResponse = [BaseResponse mj_objectWithKeyValues:responseObject];
            if (mBaseResponse.state == TOKEN_OUT || mBaseResponse.state == TOKEN_ERROR) {
                [UserService getTokenWithBlock:^(TokenResponse *_tokenResponse, NSError *error) {
                    if(_tokenResponse && _tokenResponse.state == 0){
        ...
        [UserService getTokenWithBlock:^(TokenResponse *_tokenResponse, NSError *error) {
            if(_tokenResponse && _tokenResponse.state == 0){
                ...
                completionHandler(_tokenResponse, nil, error);
            }
        }];
        return nil;
    }
